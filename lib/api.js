  ...
  methods: {
    ...
    create_job: {
      params: {
        ...
        city: { required: false, type: 'string', info: 'City name'},
      ...
      action: function(params, output) {
        ...
        var query = 'INSERT INTO `IR_properties`' +
          ...
          'JobAddress, JobCity, JobNotes, JobDateNeeded)' +
          'VALUES(NOW(), NOW(), ?, ?, ?, ?, ?, ?, ?)';
    ...
    list_job_status: {
      ...
      action: function(params, output) {
        ...
        if(params.status != null) {
          ...
          handle.find_all_jobs_by_status(params.status, function(err, result, fields) {
            if(err) {
              console.log("ERROR: ", err);
              output.fail('Invalid Request');
            }
        ...
        if(params.status == null) {
          handle.find_all_jobs_by_status("0", function(err, result, fields) {
            if(err) {
              console.log("ERROR: ", err);
              output.fail('Invalid Request');
            }
